...
class UserRepository:
    ...
    def create_user(self, user: User) -> None:
        ...
        sql = """
        INSERT INTO users (
            id, login, email, full_name, password_hash, ist_admin, aktiv,
            is_active, is_email_confirmed, email_confirmation_token,
            password_reset_token, password_reset_expires,
            created_utc, last_modified_utc, last_login_utc, role, metadata
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        params = (
            user.id,
            ...
            format_db_datetime(user.created_utc) if user.created_utc else format_db_datetime(now_utc()),
    ...
    def get_user_by_login(self, login: str) -> Optional[User]:
        """Get user by login"""
        row = self.conn.execute("SELECT * FROM users WHERE login = ?", (login,)).fetchone()
        return self._row_to_user(row) if row else None
    ...
    def update_user(self, user: User) -> None:
        ...
        sql = """
        UPDATE users SET
            login = ?, email = ?, full_name = ?, password_hash = ?, ist_admin = ?, aktiv = ?,
            is_active = ?, is_email_confirmed = ?, email_confirmation_token = ?,
            password_reset_token = ?, password_reset_expires = ?,
    ...
    def list_active_users(self) -> List[User]:
        ...
        rows = self.conn.execute("SELECT * FROM users WHERE aktiv = 1 ORDER BY full_name").fetchall()
        return [self._row_to_user(row) for row in rows]
